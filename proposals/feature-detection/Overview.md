...
if all of their component `feature`s are satisfied. If a `feature`'s name is a
member of the feature set supplied to compilation, the `feature` is satisfied
iff its `negated` field is 0, otherwise the `feature` is satisfied iff its
`negated` field is 1. A `feature` is malformed if its `negated` field has any
value besides 0 or 1.
